...
import de.uplanet.lucy.server.rtcache.filter.FieldInfoDgGuidFilter;
import de.uplanet.lucy.server.util.db.DbPreparedStatement;
import de.uplanet.lucy.server.util.db.DbQuery;
import de.uplanet.lucy.types.UP_TYPE;
        ...
        public boolean accept(FieldInfo f)
        {
          return f.getDataGroupGuid().equals(dgGuid)
                 && (jrParamName.equals(f.getSysIdent())
                     || jrParamName.equalsIgnoreCase(f.getColumnName())
                     || jrParamName.equalsIgnoreCase(f.getGuid()));
        }
      });
          ...
          catch(SQLException exc)
          {
            log.error(
                "Can't read value for datafield " + qsDataField.getName() + "("
                      + qsDataField.getGuid() + ") from Database.",
                exc);
          }
          finally
          {
